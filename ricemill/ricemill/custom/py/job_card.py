...
def repack_dialog(item, operation, qty):
    ...
    if(not opr):
        ...
        uom = frappe.get_all("UOM Conversion Detail", {'parent': item}, [
                             'uom', 'conversion_factor'])
        fields = []
        stock_uom = frappe.get_value("Item", item, 'stock_uom')
        for i in range(len(uom)):
            fields.append({'fieldname': f'uom{i+1}',
                          'label': uom[i]['uom'], 'fieldtype': 'Link', 'default': uom[i]['uom'], 'read_only': 1})
            fields.append({'fieldtype': 'Column Break',
                          'fieldname': f'column{i+1}'})
            fields.append({'fieldtype': 'Float', 'fieldname': f'conv{i+1}',
                          'default': uom[i]['conversion_factor'], 'read_only': 1, 'hidden': 1})
            fields.append({'fieldtype': 'Column Break',
                          'fieldname': f'column1{i+1}'})
            fields.append({'fieldtype': 'Float', 'fieldname': f'qty{i+1}',
                          'default': 0, 'description': f"Enter QTY in {stock_uom}"})
            fields.append({'fieldtype': 'Section Break',
                          'fieldname': f'section{i+1}'})
        fields += [
            {'fieldname': f'default_uom', 'label': "Stock UOM",
                'fieldtype': 'Link', 'default': stock_uom, 'read_only': 1},
            {'fieldtype': 'Column Break', 'fieldname': 'column_1'},
            {'fieldtype': 'Float', 'label': "Total QTY",
                'fieldname': 'total_qty', 'default': qty, 'read_only': 1},
            {'fieldtype': 'Column Break', 'fieldname': f'column_2'},
            {'fieldtype': 'Float', 'label': "Remaining QTY",
                'fieldname': 'remaining_qty', 'default': qty},
        ]
...
def make_repack(items, length, work_order_id, purpose, qty=0, sw=None, tw=None):
    ...
    parent_wo = frappe.get_value(
        "Work Order", work_order_id, 'ts_parent_work_order')
    se_name = frappe.get_value(
        "Stock Entry", {'ts_work_order': parent_wo}, 'name')
    batch_no = frappe.get_value(
        "Batch", {'reference_doctype': 'Stock Entry', 'reference_name': se_name}, 'name')
    scrap_item = ''
    ...
    job_card = frappe.get_all(
        "Job Card", filters={'work_order': work_order_id}, pluck='name')[0]
