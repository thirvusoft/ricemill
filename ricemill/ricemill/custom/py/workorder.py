...
def get_link(doc):
    job_card = frappe.get_all(
        "Job Card", filters={'work_order': doc}, pluck='name')
...
def get_linked_jobcard(name):
    job_card = frappe.get_all(
        "Job Card", filters={'work_order': name}, pluck='total_completed_qty')
    if(len(job_card)):
        return job_card[0]
...
def change_status(wo, action=None):
    jc_completed_qty = sum(frappe.get_all("Job Card", filters={
                           'work_order': wo}, pluck='total_completed_qty'))
...
def before_submit(doc, action):
    ...
    if(doc.ts_parent_work_order):
        ...
        if(frappe.db.get_value("Work Order", parent, 'status') in ['Draft', 'Not Started', 'Cancelled']):
            frappe.throw(f"Work Order "+frappe.bold(get_link_to_form("Work Order", parent)) +
                         " is not Completed. Complete that workorder to submit this workorder.")
