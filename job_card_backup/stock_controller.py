...
class StockController(AccountsController):
    ...
    def get_items_and_warehouses(self) -> Tuple[List[str], List[str]]:
        ...
        item_rows = (self.get("items") or []) + \
            (self.get("packed_items") or [])
    ...
    def check_expense_account(self, item):
        if not item.get("expense_account"):
            ...
            is_expense_account = (
                frappe.get_cached_value("Account", item.get(
                    "expense_account"), "report_type")
                == "Profit and Loss"
            )
            if (
                    self.doctype
    ...
    def delete_auto_created_batches(self):
        for d in self.items:
            ...
            frappe.db.set_value(
                "Serial No", {"batch_no": d.batch_no,
                              "status": "Inactive"}, "batch_no", None
            )
        ...
        sl_dict = frappe._dict(
            {
                ...
                "stock_uom": frappe.db.get_value(
                    "Item", args.get("item_code") or d.get(
                        "item_code"), "stock_uom"
                ),
                "incoming_rate": 0,
    ...
    def validate_warehouse(self):
        ...
        warehouses = list(set(d.warehouse for d in self.get(
            "items") if getattr(d, "warehouse", None)))
    ...
    def validate_inspection(self):
        ...
        inspection_required_fieldname = inspection_fieldname_map.get(
            self.doctype)
    ...
    def validate_qi_presence(self, row):
        ...
        if not row.quality_inspection:
            ...
            if self.docstatus == 1:
                ...
                frappe.msgprint(_(msg), title=_(
                    "Inspection Required"), indicator="blue")
    ...
    def validate_qi_submission(self, row):
        ...
        qa_docstatus = frappe.db.get_value(
            "Quality Inspection", row.quality_inspection, "docstatus")
        ...
        if not qa_docstatus == 1:
            ...
            if action == "Stop":
                frappe.throw(_(msg), title=_("Inspection Submission"),
    ...
    def validate_qi_rejection(self, row):
        ...
        qa_status = frappe.db.get_value(
            "Quality Inspection", row.quality_inspection, "status")
        ...
        if qa_status == "Rejected":
            ...
            if action == "Stop":
                frappe.throw(_(msg), title=_("Inspection Rejected"),
        ...
        if (
            ...
            for item in self.get("items"):
                if not item.target_warehouse:
                    frappe.throw(
                        _("Row {0}: Target Warehouse is mandatory for internal transfers").format(
                            item.idx)
                    )
        ...
        if (
            ...
            for item in self.get("items"):
                if not item.from_warehouse:
                    frappe.throw(
                        _("Row {0}: From Warehouse is mandatory for internal transfers").format(
                            item.idx)
                    )
    ...
    def validate_multi_currency(self):
        if self.currency != self.company_currency:
            frappe.throw(
                _("Internal transfers can only be done in company's default currency"))
        ...
        if valid_doctype:
            ...
            for item in self.get("items"):
                ...
                rule = frappe.db.get_value(
                    "Putaway Rule",
                    {"item_code": item.get(
                        "item_code"), "warehouse": item.get(warehouse_field)},
                    ["name", "disable"],
                    as_dict=True,
                )
                if rule:
                    ...
                    if self.doctype == "Stock Reconciliation":
                        ...
                        stock_qty = flt(item.transfer_qty) if self.doctype == "Stock Entry" else flt(
                            item.stock_qty)

                    rule_name = rule.get("name")
                    if not rule_map[rule_name]:
                        rule_map[rule_name]["warehouse"] = item.get(
                            warehouse_field)
                        rule_map[rule_name]["item"] = item.get("item_code")
                        rule_map[rule_name]["qty_put"] = 0
                        rule_map[rule_name]["capacity"] = get_available_putaway_capacity(
                            rule_name)
    ...
    def prepare_over_receipt_message(self, rule, values):
        ...
        message += _("Please adjust the qty or edit {0} to proceed.").format(
            rule_link)
        return message
...
def repost_required_for_queue(doc: StockController) -> bool:
    ...
    item_warehouses = [(sle.item_code, sle.warehouse)
        ...
        quality_inspection = frappe.get_doc(
            {
                ...
                # Backup
                "accepted_rejected_qty": item.get("qty")
            }
        ).insert()
...
def is_reposting_pending():
    return frappe.db.exists(
        "Repost Item Valuation", {"docstatus": 1,
...
def get_sle_entries_against_voucher(args):
    return frappe.get_all(
        "Stock Ledger Entry",
        filters={"voucher_type": args.voucher_type,
