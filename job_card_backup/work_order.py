...
def make_work_order(bom_no, item, qty=0, project=None, variant_items=None):
	...
	for i in bom.operations:
		...
		abbr = frappe.get_value("Company",wo_doc.company,'abbr')
		t_w = f'Finished Goods - {abbr}'
		wip = f'Work In Progress - {abbr}'
		wo_doc.fg_warehouse = t_w
		wo_doc.wip_warehouse = wip
		if flt(qty) > 0:
			wo_doc.qty = flt(qty)
	...
	for i in range(len(wo_names)-1):
		...
		wo.ts_parent_work_order = wo_names[i]
	...
	if(row.get("operation")):
		...
		for i in wo.operations:
			if(i.get('operation') == row.get('operation')):
				sw=i.get('ts_source_warehouse')
				tw=i.get('ts_target_warehouse')
				se_type=i.get('ts_stock_entry_type')
	doc.update({
		...
		'ts_source_warehouse': sw,
		'ts_target_warehouse': tw,
		'ts_stock_entry_type': se_type
	})
