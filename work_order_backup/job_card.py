...
class JobCard(Document):
    ...
    def validate_time_logs(self):
        ...
        if self.get("time_logs"):
            ...
            self.total_completed_qty = flt(
                self.total_completed_qty, self.precision("total_completed_qty"))
        ...
        if self.workstation:
            production_capacity = (
                frappe.get_cached_value(
                    "Workstation", self.workstation, "production_capacity") or 1
            )
    ...
    def schedule_time_logs(self, row):
        ...
        while row.remaining_time_in_mins > 0:
            args = frappe._dict(
                {"from_time": row.planned_start_time, "to_time": row.planned_end_time})
    ...
    def check_workstation_time(self, row):
        workstation_doc = frappe.get_cached_doc(
            "Workstation", self.workstation)
        if not workstation_doc.working_hours or cint(
                frappe.db.get_single_value(
                    "Manufacturing Settings", "allow_overtime")
        ):
        ...
        for i, time_slot in enumerate(workstation_doc.working_hours):
            ...
            if (
                    get_datetime(
                        row.planned_start_time) >= workstation_start_time
                    and get_datetime(row.planned_start_time) <= workstation_end_time
            ):
                time_in_mins = time_diff_in_minutes(
                    workstation_end_time, row.planned_start_time)
                ...
                if time_in_mins > row.remaining_time_in_mins:
                    ...
                    row.planned_end_time = add_to_date(
                        row.planned_start_time, minutes=time_in_mins)
                ...
                if total_idx != (i + 1) and row.remaining_time_in_mins > 0:
                    row.planned_start_time = datetime.datetime.combine(
                        start_date, get_time(
                            workstation_doc.working_hours[i + 1].start_time)
                    )
        ...
        if row.remaining_time_in_mins > 0:
            ...
            row.planned_start_time = datetime.datetime.combine(
                start_date, get_time(
                    workstation_doc.working_hours[0].start_time)
            )
    ...
    def set_employees(self, employees):
        for name in employees:
            self.append("employee", {"employee": name.get(
                "employee"), "completed_qty": 0.0})
    ...
    def update_sub_operation_status(self):
        ...
        for time_log in self.time_logs:
            if time_log.operation not in operation_wise_completed_time:
                operation_wise_completed_time.setdefault(
                    time_log.operation,
                    frappe._dict(
                        {"status": "Pending", "completed_qty": 0.0,
                            "completed_time": 0.0, "employee": []}
                    ),
                )
        ...
        for row in self.sub_operations:
            operation_deatils = operation_wise_completed_time.get(
                row.sub_operation)
            if operation_deatils:
                ...
                if operation_deatils.employee:
                    row.completed_time = row.completed_time / \
                        len(set(operation_deatils.employee))

                    if operation_deatils.completed_qty:
                        row.completed_qty = operation_deatils.completed_qty / \
                            len(set(operation_deatils.employee))
            else:
                row.status = "Pending"
    ...
    def update_corrective_in_work_order(self, wo):
        ...
        for row in frappe.get_all(
                ...
                filters={"is_corrective_job_card": 1,
                         "docstatus": 1, "work_order": self.work_order},
        ):
            wo.corrective_operation_cost += flt(
                row.total_time_in_mins) * flt(row.hour_rate)
    ...
    def validate_produced_quantity(self, for_quantity, wo):
        ...
        if wo.produced_qty > for_quantity:
            first_part_msg = _(
                ...
                frappe.bold(_("Job Card")), frappe.bold(
                    self.name), frappe.bold(self.production_item)
            )
    ...
    def get_current_operation_data(self):
        return frappe.get_all(
            "Job Card",
            fields=["sum(total_time_in_mins) as time_in_mins",
    ...
    def set_transferred_qty_in_job_card_item(self, ste_doc):
        ...
        def _validate_over_transfer(row, transferred_qty):
            ...
            required_qty = frappe.db.get_value(
                "Job Card Item", row.job_card_item, "required_qty")
    ...
    def set_status(self, update_status=False):
        ...
        self.status = {0: "Open", 1: "Submitted",
                       2: "Cancelled"}[self.docstatus or 0]
    ...
    def validate_work_order(self):
        if self.is_work_order_stopped():
            frappe.throw(
                _("You can't make any changes to Job Card since Work Order is stopped."))
...
def make_stock_entry(source_name, target_doc=None):
    ...
    def set_missing_values(source, target):
        ...
        pending_fg_qty = flt(source.get("for_quantity", 0)) - \
            flt(source.get("transferred_qty", 0))
